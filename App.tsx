...
const findCurrentLineIndex = (lyricLines: LyricLine[], time: number): number => {
  if (!lyricLines || lyricLines.length === 0) return -1;
  const loopTime = time % MOCK_DURATION;
  let lineIndex = -1;
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 先用预估时长初始化滚动时间（进入第 5 轮，便于向上滚动回看）
      const initialStartTime = MOCK_DURATION * 5;
      setScrollTime(initialStartTime);
      setCurrentTime(0);
      if (audioRef.current) audioRef.current.currentTime = 0;
      // 其余真实时长会在 onLoadedMetadata 中更新
      // 这里无需手动清理 URL 对象
      console.info('[Audio] Source set to', realAudioPath);
    } catch (error) {
  ...
  const handleSeek = useCallback((absoluteTime: number) => {
    ...
    seekTimeoutRef.current = setTimeout(() => { isSeekingRef.current = false; }, 200);
  ...
  const handleLoadedMetadata = useCallback((e: React.SyntheticEvent<HTMLAudioElement>) => {
    ...
    try {
      ...
      // 将滚动时间切换到基于真实时长的第 5 轮，以维持既有的交互体验
      ...
      if (pendingAbs != null) {
        ...
        try {
          ...
          pendingSeekAbsoluteTimeRef.current = null;
          console.info('[Audio] Applied pending seek after metadata load:', pendingAbs);
        } catch (seekErr) {
          console.error('[Audio] Failed to apply pending seek after metadata load:', seekErr);
        }
      }
      console.info('[Audio] Metadata loaded. Duration =', realDuration, 'seconds');
    } catch (err) {
  ...
  const handleCanPlay = () => {
    try {
      ...
      if (pendingAbs != null && audio) {
        ...
        pendingSeekAbsoluteTimeRef.current = null;
        console.info('[Audio] Applied pending seek at canplay:', pendingAbs);
      }
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#202734] font-sans overflow-hidden">
        <audio
